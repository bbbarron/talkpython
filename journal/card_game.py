...
class Game(Scene):
    ...
    def deal(self):
        ...
        self.cards = []
        self.selected = []
        card_size = (175 if self.size.w > 700 else 64)
        width = (card_size + 5) * 4
        offset = Point((self.size.w - width) / 2, (self.size.h - width)
                       / 2)
        for i in xrange(len(images)):
            (x, y) = (i % 4, i / 4)
            card = Layer(Rect(offset.x + x * (card_size + 5), offset.y
                              + y * (card_size + 5), card_size, card_size))
